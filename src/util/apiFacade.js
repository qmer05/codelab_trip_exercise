...
object when you do)*/
  ...
  const fetchDataTrips = () => {
    const options = makeOptions("GET", true); // True adds the token
    return fetch(URL + "/trips", options).then(handleHttpErrors);
  };

  const fetchDataGuides = () => {
    const options = makeOptions("GET", true); // True adds the token
    return fetch(URL + "/guides", options).then(handleHttpErrors);
  };
